...
CONTAINER="zphisher"
IMAGE="ZainNaveed34211/zainnaveed:latest"
IMG_MIRROR="ghcr.io/ZainNaveed34211/zainnaveed:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/zainnaveed/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/zainnaveed/auth/ --name "$CONTAINER" "$IMAGE"
